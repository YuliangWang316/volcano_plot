...
g = ggplot(data=gene_list, aes(x=logFC, y=-log10(padj),color=threshold)) + geom_point(alpha=0.6, size=0.3)  + xlim(c(-7, 7)) + ylim(c(0, 50)) +xlab("log2 fold change") + ylab("-log10 p-value")+geom_text(mapping=aes(label=rownames(Spgenes)),data = Spgenes,check_overlap = TRUE,color="Black") + theme_set(theme_bw()) + theme(panel.grid.major=element_line(colour=NA)) + scale_color_manual(values = Mycolors)
print(g)
dev.off()
